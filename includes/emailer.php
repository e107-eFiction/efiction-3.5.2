...
function sendemail($to_name,$to_email,$from_name,$from_email,$subject,$message,$type="plain",$cc="",$bcc="") {
	...
	if (!ereg("^[-!#$%&'*+\./0-9=?A-Z^_`a-z{|}~]+@{1}([a-z0-9]{1}[a-z0-9-]*[a-z0-9]{1}\.{1})+([a-z]+\.){0,1}([a-z]){2,4}$", $to_email)) return false; // Bad e-mail
	if (!ereg("^[-!#$%&'*+\./0-9=?A-Z^_`a-z{|}~]+@{1}([a-z0-9]{1}[a-z0-9-]*[a-z0-9]{1}\.{1})+([a-z]+\.){0,1}([a-z]){2,4}$", $from_email)) return false; // Bad e-mail
