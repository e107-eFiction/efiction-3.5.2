...
if(!defined("_CHARSET")) exit( );
	...
	if(isset($_POST['submit'])) {
		...
		if($type == "SE") $query2 = "SELECT uid FROM ".TABLEPREFIX."fanfiction_series WHERE seriesid = '".$reviews['item']."'";
			...
			if(empty($reviews['chapid'])) $query2 = "SELECT uid, coauthors FROM ".TABLEPREFIX."fanfiction_stories WHERE sid = '".$reviews['item']."' LIMIT 1";
			else $query2 = "SELECT ch.uid, st.coauthors FROM ".TABLEPREFIX."fanfiction_chapters as ch LEFT JOIN ".TABLEPREFIX."fanfiction_stories as st ON ch.sid = st.sid WHERE chapid = '".$reviews['chapid']."' LIMIT 1";
		}
		else { 
		...
		if(USERUID == $user['uid'] || (!empty($user['coauthors']) && in_array(USERUID, explode(",", $user['coauthors'])))) {
			if($type == "ST") {
